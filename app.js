...
function bindAuthEvents() {
    ...
    const forgotEmailInput = $('forgot-email-input');
    const forgotModalError = $('forgot-modal-error');
    const forgotModalSuccess = $('forgot-modal-success');

    function openForgotModal() {
        if (forgotModalError) { forgotModalError.style.display = 'none'; forgotModalError.textContent = ''; }
        if (forgotModalSuccess) { forgotModalSuccess.style.display = 'none'; forgotModalSuccess.textContent = ''; }
    ...
    if (btnForgotGenerate) {
        btnForgotGenerate.addEventListener('click', async () => {
            ...
            if (forgotModalError) { forgotModalError.style.display = 'none'; }
            if (forgotModalSuccess) { forgotModalSuccess.style.display = 'none'; }

            if (!username || !email) {
                if (forgotModalError) {
                    forgotModalError.textContent = 'Please enter both username and email.';
                    forgotModalError.style.display = 'block';
                }
                return;
            }

            btnForgotGenerate.disabled = true;
            btnForgotGenerate.textContent = 'SENDING...';
            ...
            try {
                ...
                if (forgotModalSuccess) {
                    forgotModalSuccess.textContent = data.message || 'Reset link sent to your email address!';
                    forgotModalSuccess.style.display = 'block';
                }
            } catch (err) {
                if (forgotModalError) {
                    forgotModalError.textContent = err.message;
                    forgotModalError.style.display = 'block';
                }
            } finally {
                btnForgotGenerate.disabled = false;
                btnForgotGenerate.textContent = 'SEND RESET LINK';
            }
        });
    }
    ...
    // Account Settings Picture Upload
    const accountPicInput = $('account-pic-input');
    if (accountPicInput) {
        accountPicInput.addEventListener('change', e => {
            const file = e.target.files[0];
            if (file) {
                const reader = new FileReader();
                reader.onload = ev => {
                    const img = $('account-pic-img');
                    const fallback = $('account-pic-fallback');
                    if (img) {
    ...
    if (accountSettingsForm) {
        accountSettingsForm.addEventListener('submit', async e => {
            ...
            const picImg = $('account-pic-img');
            const profilePic = picImg && picImg.style.display !== 'none' && !picImg.src.endsWith('profile.png') ? picImg.src : currentUser.profilePic;
            ...
            try {
                const res = await fetch('/api/auth/profile', {
                    ...
                    body: JSON.stringify({ email, phone, profilePic })
                });
...
function updateSidebarProfile() {
    ...
    const sidebarFallback = $('sidebar-avatar-fallback');
    const headerImg = $('header-avatar-img');
    const headerFallback = $('header-avatar-fallback');

    if (currentUser.profilePic) {
        if (sidebarImg) { sidebarImg.src = currentUser.profilePic; sidebarImg.style.display = 'block'; }
        if (sidebarFallback) sidebarFallback.style.display = 'none';
        if (headerImg) { headerImg.src = currentUser.profilePic; headerImg.style.display = 'block'; }
...
function switchTab(tabId) {
    ...
    else if (tabId === 'account')   renderAccountTab();
...
function renderDashboardFeed(expiringSoon, items) {
    ...
    // Sort items by date added descending and limit to 3 recent items
    const recentItems = [...items].sort((a,b) => new Date(b.dateAdded) - new Date(a.dateAdded)).slice(0, 3);

    if (recentItems.length === 0) {
        list.innerHTML = '<div style="padding: 20px; text-align: center; color: var(--text-muted);">No items added yet.</div>';
        return;
    }
            ...
            <div class="feed-card">
                ...
                <div class="feed-card-content" style="flex: 1;">
                    <div class="feed-card-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                        ...
                        <span class="feed-card-expiry ${expiryClass}" style="font-size: 12px; font-weight: 600;">${expiryText}</span>
                    </div>
                    <span class="feed-card-sub" style="font-size: 13px; color: var(--text-muted); display: block; margin-bottom: 8px;">Added: ${item.dateAdded}</span>
                    <div class="feed-card-tags">
                        <span class="feed-tag">${escHTML(item.category)}</span>
                        <span class="feed-tag">${escHTML(item.storage)}</span>
                    </div>
                </div>
            </div>
        `;
...
function renderAccountTab() {
    ...
    if (roleBadge) {
        roleBadge.textContent = currentUser.role === 'admin' ? 'Administrator' : 'Regular User';
        roleBadge.style.background = currentUser.role === 'admin' ? 'var(--color-danger)' : 'var(--color-primary)';
    }
    ...
    // Set profile picture
    const picImg = $('account-pic-img');
    const picFallback = $('account-pic-fallback');
    if (currentUser.profilePic) {
        if (picImg) { picImg.src = currentUser.profilePic; picImg.style.display = 'block'; }
        if (picFallback) picFallback.style.display = 'none';
    } else {
        if (picImg) { picImg.src = 'images/profile.png'; picImg.style.display = 'none'; }
