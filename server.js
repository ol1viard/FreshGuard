...
nodemailer.createTestAccount().then(account => {
    ...
    console.log('Ethereal Email initialized for testing. Test emails will be logged to console.');
}).catch(err => {
    console.error('Failed to initialize test email account:', err);
});
// ---------------------------------------------------------------------------
...
async function initDb() {
    try {
        ...
        try { await dbRun('ALTER TABLE users ADD COLUMN profilePic TEXT'); } catch(_) {}
...
app.post('/api/auth/signup', async (req, res) => {
    ...
    try {
        ...
        res.json({ token, user: { username, role, provider: 'local', email: null, phone: null, profilePic: null } });
    } catch (err) {
...
app.post('/api/auth/signin', async (req, res) => {
    ...
    try {
        ...
        res.json({ token, user: { username: user.username, role: user.role, provider: 'local', email: user.email, phone: user.phone, profilePic: user.profilePic } });
    } catch (err) {
...
app.put('/api/auth/profile', authenticateToken, async (req, res) => {
    ...
    try {
        await dbRun('UPDATE users SET email = ?, phone = ?, profilePic = ? WHERE username = ?', 
            [email || null, phone || null, profilePic || null, req.user.username]);
...
app.post('/api/auth/forgot-password', async (req, res) => {
    ...
    try {
        ...
        if (!user || user.email !== email) {
            ...
            return res.json({ message: 'If the username and email match an account, a reset link has been sent to your email.' });
        }
        if (user.provider === 'google') {
            return res.status(400).json({ error: 'Google accounts cannot be reset manually' });
        }
        ...
        await dbRun('UPDATE users SET resetToken = ?, resetTokenExpiry = ? WHERE username = ?', [token, expiry, user.username]);
        
        // Generate reset link
        const resetLink = `${req.protocol}://${req.get('host')}/?user=${encodeURIComponent(user.username)}&token=${encodeURIComponent(token)}`;

        if (transporter) {
            const info = await transporter.sendMail({
                from: '"FreshGuard Admin" <admin@freshguard.local>',
                to: email,
                subject: 'Password Reset Request',
                text: `Hello ${user.username},\n\nPlease click the following link to reset your password:\n${resetLink}\n\nThis link will expire in 1 hour.`,
                html: `<p>Hello ${user.username},</p><p>Please click the following link to reset your password:</p><p><a href="${resetLink}">${resetLink}</a></p><p>This link will expire in 1 hour.</p>`
            });
            console.log('Password Reset Email sent! Preview URL: %s', nodemailer.getTestMessageUrl(info));
        } else {
            console.log('Password Reset Link (transporter not ready):', resetLink);
        }

        res.json({ message: 'If the username and email match an account, a reset link has been sent to your email.' });
    } catch (err) {
